...
public class StompHandler implements ChannelInterceptor {
	...
	public Message<?> preSend(Message<?> message, MessageChannel channel) {
		...
		// TODO token 검증, 프론트에서 넘겨줄지 말지 미정.
		 ...
		 */
			...
			/*
			 * 채팅목록 sub인지 채팅 메시지 내 sub인지 구분해야한다. 
			 ...
			 * chatId를 가져온다.
			 */
			String chatId = chatService.getChatId(
					Optional.ofNullable((String) message.getHeaders().get("simpDestination")).orElse("Invalid"));
			if (!"Invalid".equals(chatId)) {
				 ...
				 * messageId를 비교해서 그만큼 socket으로 갱신되었다고 뿌려주기
				 * /sub/chat/unreadCnt/{chatId}
				 */
		 ...
		 */
			...
			/*
			 * chatId가 null 이 아니라면 
			 * sub/chat/room/{chatId}의 DISCONNECT이기 때문에 Hash에서 지워준다.
			 */
			if (chatId != null) {
				/*
